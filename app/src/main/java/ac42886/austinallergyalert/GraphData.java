...
public class GraphData extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_toggle_touch_zoom) {
            ...
            // create the toast
            Context context = getApplicationContext();
            Toast.makeText(context, "IsZoomEnabled " + chart.isZoomEnabled(), Toast.LENGTH_SHORT).show();
            return true;
        }
    ...
    private void addLineToData() {
        if (data.getLines().size() >= maxNumberOfLines) {
            ...
            // create the toast
            Context context = getApplicationContext();
            Toast.makeText(context, "Samples app uses max 4 lines!", Toast.LENGTH_SHORT).show();
            return;
        } else {
            ++numberOfLines;
        }
